...
function canonical(name=''){
  const aliases={
    manunited:'Man Utd',manutd:'Man Utd',mancity:'Man City',freiburg:'Freiburg',augsburg:'Augsburg',
    cologne:'1. FC Cologne',schalke:'Schalke',monchengladbach:'Borussia Monchengladbach',twente:'FC Twente Enschede',
    ...
    arsenal:'Arsenal',chelsea:'Chelsea',hullcity:'Hull City',leeds:'Leeds United',leedsunited:'Leeds United',
    newcastle:'Newcastle',newcastleunited:'Newcastle',standardliege:'Standard Liege',adodenhaag:'ADO Den Haag',
    fortunasittard:'Fortuna Sittard',lommelsk:'Lommel SK',royalcharleroisc:'Royal Charleroi SC',spartotterdam:'Sparta Rotterdam',
    sttruindensevv:'St. Truidense VV',stmirrenfc:'St Mirren FC',motherwellfc:'Motherwell FC',
...
function toEvidence(row,source,index){
  const parsed=parseScore(row.score); if(!parsed)return null;
  const market=row.market||row.pick||'Pattern review';
  return {source,index,date:row.date||'',home:canonical(row.home),away:canonical(row.away),homeScore:parsed.homeScore,awayScore:parsed.awayScore,market,pick:row.pick||market,odds:row.odds,result:row.result||row.status};
}
...
function buildProfiles(){
  ...
  for(const e of ALL_EVIDENCE){
    for(const club of [e.home,e.away]){
      if(!profiles[club])profiles[club]={club,entries:[],wins:0};
      profiles[club].entries.push(e);
      if(marketWon(e))profiles[club].wins++;
    }
  }
  for(const p of Object.values(profiles)){
    p.entries.sort((a,b)=>String(b.date).localeCompare(String(a.date)));
    const recent=p.entries.slice(0,5);
    p.sample=p.entries.length;
    p.recentSample=recent.length;
    p.hitRate=p.sample?Math.round((p.wins/p.sample)*1000)/10:0;
    p.latest=recent[0]||null;
    p.recentWins=recent.filter(marketWon).length;
    p.recentRate=recent.length?Math.round((p.recentWins/recent.length)*1000)/10:0;
    p.pattern=p.latest?.market||p.latest?.pick||'Pattern review';
  }
  return profiles;
}
...
function signalFor(home,away){
  const hp=profileFor(home), ap=profileFor(away), p=bestProfile(home,away);
  if(!p)return 'Cautious';
  const rate=p.recentRate;
  if(rate>=80 && p.sample>=2)return 'Strong';
  if(rate>=60 || (hp&&ap&&Math.abs(hp.recentRate-ap.recentRate)<15))return 'Balanced';
  return 'Cautious';
}
...
export async function GET(){
  ...
  const decorate=m=>{const p=bestProfile(m.home,m.away),lt=labelTime(m.date);return{...m,...lt,prediction:predictionFor(m.home,m.away),source:sourceFor(m.home,m.away),signal:signalFor(m.home,m.away),lastResult:lastResultFor(m.home)||lastResultFor(m.away),lastMatch:p?.latest?`${p.latest.home} ${p.latest.homeScore}-${p.latest.awayScore} ${p.latest.away}`:null,tracked:Boolean(p),evidence:{sample:p?.sample||0,hitRate:p?.hitRate||0,recentSample:p?.recentSample||0,recentRate:p?.recentRate||0,latestPattern:p?.pattern||null}}};
  };
  const matches=upcoming.filter(m=>profileFor(m.home)||profileFor(m.away)).map(decorate);
  ...
  const datasetStats={original27:FOUNDATION.length,latest29:RECENT_29.length,combinedEvidence:ALL_EVIDENCE.length,latestCompleted:RECENT_29.filter(r=>r.status!=='Not Started').length,latestWon:RECENT_29.filter(r=>r.status==='Won').length,latestLost:RECENT_29.filter(r=>r.status==='Lost').length};
  return NextResponse.json({generatedAt:now.toISOString(),window:{from:friday.toISOString(),to:sunday.toISOString()},matches,recentReview,futureMatches,trackedClubs:Object.keys(PROFILES),datasetStats,provider:'MatchPattern datasets + ESPN public soccer schedule feed'},{headers:{'Cache-Control':'no-store'}});
}
